...
| `storage_load_turns` | 加载会话的所有 Turn（含附件图片加载为 data URL） | storage service |
| `storage_append_turn` | 追加一条 Turn（含可选 image_data，存储为附件文件） | storage service |
...
| `toggle_always_on_top` | 切换侧边窗置顶状态 | SidebarApp |
...
- `context.ts` 管理线程上下文：将 Turn[] 转换为 LLM ChatMessage[]，每条 user Turn 优先使用自身 `image_data` 字段携带截图，仅当 Turn 无 `image_data` 且为首条 user Turn 时回退到全局 `threadImageData`；支持 tool 角色消息和 tool_calls 字段的转换；定义上下文长度上限（`MAX_CONTEXT_CHARS=16000`，约 4000 tokens）和 Turn 上限（`MAX_CONTEXT_TURNS=20`）；截断策略保留 system prompt + 首条 user（含截图锚点）+ 首条 assistant 回复作为锚点，然后保留最近 4 条 Turn，超限时从中间丢弃。
...
- 追问时截图上下文：每条 user Turn 持久存储 `image_data`（附件路径），加载时由 Rust 后端将路径读回为 data URL。`turnsToMessages` 从每条 Turn 自身的 `image_data` 读取截图，仅当 Turn 无 `image_data` 且为首条 user Turn 时回退到 `threadImageData`。全局 `threadImageData` 仅用于当前新截图的即时发送场景。截图附件存储在 `attachments/{turn_id}.png`，以 base64 解码后写入文件，加载时读回并编码为 `data:image/png;base64,...` data URL。
...
app_data_dir/
  ...
  conversations/
    ...
    {conversation-id}/
      ...
      messages.json               # Turn[]
      attachments/                # 截图附件（每条 user Turn 的截图存为 {turn_id}.png）
```
...
| image_data | string \| null | 截图附件引用路径（如 `attachments/{turn_id}.png`），存储时为相对路径，加载时由 Rust 后端读回为 data URL |
| created_at | string (RFC3339) | 创建时间 |
...
- **ConversationContext**：当前活跃会话状态。使用 `INITIAL_STATE` 常量统一重置，包含：会话 ID、Turn 列表、流式文本、线程截图引用 `threadImageData`、视图模式、`skillCallInfo` 工具调用状态、`sources` 来源引用列表、`lastError` 分类错误。action 包括 `setLastError` / `clearError` 用于错误横幅联动。关闭窗口时仅隐藏窗口不清空会话状态，用户主动点击 ChatView 关闭按钮才回退到空状态。
...
`threadImageData` 在 `ConversationContext` 中保存当前线程关联的截图数据。截图数据同时通过 `Turn.image_data` 持久化存储为附件文件，确保会话历史中的截图不丢失。`turnsToMessages` 从每条 Turn 的 `image_data` 读取截图，仅在 Turn 无 `image_data` 且为首条 user Turn 时回退到 `threadImageData`。窗口关闭时只隐藏窗口不清空会话状态，用户主动点击 ChatView 关闭按钮才回退到空状态。`SidebarApp` 中 `pendingCaptureImage` 为待确认的截图预览状态，两者职责分离。

侧边窗默认不置顶（`always_on_top: false`），用户可通过标题栏置顶按钮切换置顶状态，满足 SRS FR-021（回答窗必须支持置顶、关闭、收起）和 FR-004（不得频繁打扰用户）。
