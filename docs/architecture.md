...
| Skill 调用 | Tavily API 不可用 / Key 未配置 / 超时 | `SkillError` 包装，UI 显示错误状态，用户可重试 | FR-061 |
...
- **ConversationContext**：当前活跃会话状态（会话 ID、Turn 列表、流式文本、线程截图引用 `threadImageData`、视图模式、`skillCallInfo` 工具调用状态、`sources` 来源引用列表）。
