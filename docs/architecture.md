...
| 设置弹窗 | `src/components/SettingsModal.tsx` | Provider 选择、API Key / URL / 模型配置 |
...
src/services/llm/
  ...
  init.ts              # LLMClient 初始化（设置 UI 优先 → 环境变量回退）
...
- `prompt.ts` 构建老师式系统指令（SRS FR-033：这是什么/为什么重要/如何理解），追问时附带上下文衔接提示；`buildUserContent` 返回 `MessageContent` 类型（string 或多模态数组），统一与 LLM 层类型系统。
...
- **ConversationContext**：当前活跃会话状态（会话 ID、Turn 列表、流式文本、线程截图引用 `threadImageData`、视图模式）。
- **SettingsContext**：应用设置状态（默认 Provider、各 Provider 的 API Key / Base URL / 模型），初始化时从 Rust 后端加载，保存时写入本地文件并重建 LLM 客户端。
